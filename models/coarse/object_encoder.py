...
class ObjectEncoder(torch.nn.Module):
    def __init__(self, embed_dim: int, known_classes: List[str], known_colors: List[str], args):
        ...
        pointnet_state = torch.load(args.pointnet_path, map_location="cpu")
        if isinstance(pointnet_state, dict) and "model_state" in pointnet_state:
            pointnet_state = pointnet_state["model_state"]
        target_state = self.pointnet.state_dict()
        load_state = {}
        for key, value in pointnet_state.items():
            clean_key = key
            for prefix in ("object_encoder.pointnet.", "pointnet."):
                if clean_key.startswith(prefix):
                    clean_key = clean_key[len(prefix):]
            if clean_key in target_state and target_state[clean_key].shape == value.shape:
                load_state[clean_key] = value
        missing, unexpected = self.pointnet.load_state_dict(load_state, strict=False)
        print(
            f"Loaded PointNet weights from {args.pointnet_path}: "
            f"{len(load_state)} tensors, {len(missing)} missing, {len(unexpected)} unexpected."
        )
