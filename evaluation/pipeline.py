...
CITYLOC_K_VAL_SCENES = ["2013_05_28_drive_0010_sync"]
CITYLOC_K_TEST_SCENES = [
...
def _discover_scene_names(base_path):
    poses_dir = osp.join(base_path, "poses")
    return sorted(
        osp.splitext(name)[0]
...
def _resolve_eval_scene_names(args):
    if args.scene_names:
        return args.scene_names
    if args.eval_split == "cityloc_k_val":
        return CITYLOC_K_VAL_SCENES
    if args.eval_split == "cityloc_k_test":
        return CITYLOC_K_TEST_SCENES
    if args.eval_split == "cityloc_c_val":
        return _discover_scene_names(args.base_path)
    if args.eval_split == "kitti360_val":
        return SCENE_NAMES_VAL
    if args.eval_split == "kitti360_test":
        return SCENE_NAMES_TEST
    return SCENE_NAMES_TEST if args.use_test_set else SCENE_NAMES_VAL


def _target_cell_id(pose, args):
    return str(getattr(pose, args.target_cell_attr, pose.cell_id))
...
def encode_retrieval_embeddings(model, dataloader, args):
    ...
    db_cell_ids = np.zeros(len(cells_dataset), dtype="<U128")

    text_encodings = np.zeros((len(dataloader.dataset), model.embed_dim), dtype=np.float32)
    query_cell_ids = np.zeros(len(dataloader.dataset), dtype="<U128")
    ...
    for batch in tqdm.tqdm(dataloader):
        ...
        query_cell_ids[index_offset : index_offset + batch_size] = np.array(
            [_target_cell_id(pose, args) for pose in batch["poses"]]
        )
...
def run_coarse_fused(model_primary, model_secondary, dataloader, args):
    ...
    for query_idx in range(len(primary["text_encodings"])):
        ...
        retrieved_cells = [cells_dict[cell_id] for cell_id in retrieved_cell_ids]
        retrieved_cell_poses = [cell.get_center()[0:2] for cell in retrieved_cells]
        dists = np.linalg.norm(target_pose_w - retrieved_cell_poses, axis=1)
        pose = dataloader.dataset.all_poses[query_idx]
        pose_scene_name = str(getattr(pose, "eval_scene_name", getattr(pose, "scene_name", "")))
        cell_scene_names = np.array([str(getattr(cell, "scene_name", "")) for cell in retrieved_cells])
        if pose_scene_name:
...
def run_coarse(model, dataloader, args):
    ...
    if args.rerank_topn > 0:
        db_cell_ids = np.array([cell.id for cell in dataloader.dataset.all_cells], dtype="<U128")
        reranked = {}
        ...
        query_poses_w = np.array([pose.pose_w[0:2] for pose in dataloader.dataset.all_poses])
        query_cell_ids = np.array(
            [_target_cell_id(pose, args) for pose in dataloader.dataset.all_poses],
            dtype="<U128",
        )
    ...
    for query_idx in range(len(retrievals)):
        ...
        target_pose_w = query_poses_w[query_idx]
        retrieved_cells = [all_cells_dict[cell_id] for cell_id in retrieved_cell_ids]
        retrieved_cell_poses = [cell.get_center()[0:2] for cell in retrieved_cells]
        dists = np.linalg.norm(target_pose_w - retrieved_cell_poses, axis=1)
        pose_scene_name = str(getattr(pose, "eval_scene_name", getattr(pose, "scene_name", "")))
        cell_scene_names = np.array([str(getattr(cell, "scene_name", "")) for cell in retrieved_cells])
...
if __name__ == "__main__":
    ...
    scene_names = _resolve_eval_scene_names(args)
    print("Evaluation scenes:", scene_names)
    dataset_retrieval = Kitti360CoarseDatasetMulti(
        args.base_path, scene_names, transform, shuffle_hints=False, flip_poses=False,
    )
