...
def _pose_scene_name(pose):
    return str(
        getattr(
            pose,
            "eval_scene_name",
            getattr(pose, "scene_name", str(pose.cell_id).rsplit("_", 1)[0]),
        )
    )


def _cell_scene_name(cell):
    return str(getattr(cell, "scene_name", str(cell.id).rsplit("_", 1)[0]))
...
def calc_sample_accuracies(pose, top_cells, pos_in_cells, top_k, threshs):
    ...
    pose_scene_name = _pose_scene_name(pose)
    cell_scene_names = np.array([_cell_scene_name(cell) for cell in top_cells])
